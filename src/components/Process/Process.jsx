    ...
    const rendersteps = step.map(item => {
        return (
            <div className={`flex-1 basis-[300px] ${item.id % 2=== 0 ? 'md:-mt-100': ""} `   } >
                <span className='flex  justify-center items-center rounded-full mx-auto w-18 h-18 text-8xl bg-zinc-800 text-white outline-[3px] outline-offset-7 outline-zinc-800 outline-dashed'>{item.number}</span>
    ...
    return (
        <section>
            <div className='max-w-[1400px] mx-auto py-20 px-10'>
                ...
                <div className='flex flex-wrap gap-y-17 mt-20 items-center justify-center md:mt-20 mt-10 md:pt-50'>
                    {rendersteps}
                </div>
            </div>
        </section>
